	...
	public class DesertionCampaignBehavior : CampaignBehaviorBase
	{
		...
		private static void CheckDesertionForParty(MobileParty mobileParty)
		{
			...
			if (troopsToDesert.Count > 0)
			{
				...
				{
					Debug.Print(string.Format("[High Desertion Alert]  Deserted troop count for party: {0} is: {1}, remaining troop count: {2}, all member count before desertion: {3}", new object[]
					{
						mobileParty.StringId,
						troopsToDesert.TotalManCount,
						numberOfAllMembers2,
						numberOfAllMembers
